...
public class CodeTest
{
    ...
    public CodeTest()
    {
        ...
        _blockSize = PixelsPerAline * AlinesPerFrame * _floatSize;
        _filePath = @"J:\Data_2025\20250326_Jurkat4\Day0_Control_Pos1(bottom)\Data.bin";
    }

    public float[,] GetFloatData() => _floatData; // 暴露数据给测试.   问一下这一行什么意思

    public async Task InitializeAsync()
        {
            using var fileStream = new FileStream(_filePath, FileMode.Open, FileAccess.Read);
            var fileLength = fileStream.Length;
            var totalBlocks = fileLength / _blockSize;  // 检查过了是4096没问题
            var currentBlock = 0;
            var buffer = ArrayPool<byte>.Shared.Rent(_blockSize);

            // var bytesRead = await fileStream.ReadAsync(buffer, 0, _blockSize); //第 1 块，读取了 819200 字节
            _floatData = ConvertBytesToFloat(buffer, PixelsPerAline, AlinesPerFrame);
            ArrayPool<byte>.Shared.Return(buffer); // 记得归还缓冲区
            // Console.WriteLine($"buffer's length is: {buffer.Length}; ");
            // Console.WriteLine($"Block size is: {_blockSize}; ");
            // Console.WriteLine($"height * width * sizeof(float) is: {PixelsPerAline} x {AlinesPerFrame} x {sizeof(float)}; ");
            // Console.WriteLine($"floatData size is: {floatData.GetLength(0)}, {floatData.GetLength(1)}");
            Console.Write("floatData[0, 0..9]: ");
            for (int col = 0; col < 10; col++) { Console.Write($"{_floatData[50, col]} "); }  // 逐行打印
        }

    private float[,] ConvertBytesToFloat(byte[] buffer, int width, int height)
    {
        var result = new float[height, width];
        Buffer.BlockCopy(buffer, 0, result, 0, _blockSize);
        return result;
    }
